...
<script type="text/javascript">
	function deleteComment(){
		...
		if(selectedRows.length==0){
		 ...
		 $.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
				if(r){
					$.post("${ctx}/admin/removeComment.action",{ids:ids},function(result){
						if(result.success){
	...
	function formatBlogTitle(val,row){
		if(val==null){
			...
			return "<a target='_blank' href='${ctx}/blog/articles/"+val.id+".action'>"+val.title+"</a>";
		}
	}
...
<table id="dg" title="评论管理" class="easyui-datagrid"
   ...
   url="${ctx}/admin/commentList.action" fit="true" toolbar="#tb">
   <thead>
   	<tr>
